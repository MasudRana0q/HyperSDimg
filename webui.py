...
try:
    import modules.gradio_hijack as grh
    if not (hasattr(grh.Image, 'change') or hasattr(grh.Image, 'upload')):
        raise RuntimeError('gradio_hijack Image has no change/upload')
except Exception:
    grh = gr
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            with gr.Tab("Generation"):
                with gr.Row():
                    progress_window = gr.Image(label='Preview', show_label=True, visible=False, height=768,
                                           elem_id='progress_window', elem_classes='progress_window')
            ...
            with gr.Row(elem_classes='type_row'):
                ...
                with gr.Column(scale=3, min_width=0):
                    # গ্রাডিও ৪.০+ এ Button এ label প্যারামিটার নেই, value-ই লেবেল হিসেবে কাজ করে
                    ...
                    # গ্রাডিও ৪.০+ এ _js প্যারামিটার সাপোর্ট করে না, তাই রিমুভ করা হয়েছে
                    stop_button.click(stop_clicked, inputs=currentTask, outputs=currentTask, queue=False, show_progress=False)
        ...
        if hasattr(uov_input_image, 'upload'):
            uov_input_image.upload(trigger_uov_describe, inputs=[desc_method, uov_input_image, prompt],
                           outputs=[prompt, style_selections], show_progress=True, queue=True)
        else:
            uov_input_image.change(trigger_uov_describe, inputs=[desc_method, uov_input_image, prompt],
                           outputs=[prompt, style_selections], show_progress=True, queue=True)
